...
def analyze_user_journey(url: str, max_pages: int = 3, max_depth: int = 1) -> UserJourneyReport:
    ...
    logger.info(f"Step 8: Creating comprehensive microbehaviors (targeting 6-9 behaviors)")
    journey_steps: List[JourneyStep] = []
    
    # Extract comprehensive microbehaviors - aim for more detailed actions
    all_behaviors = []
    ...
    for idx, step_data in enumerate(journey_steps_raw, start=1):
        ...
        # Add ALL user actions (don't limit - these are the key microbehaviors)
        for action in user_actions:
            if action and len(action.strip()) > 3:  # Skip empty or very short actions
        ...
        # Add content reading behaviors for ONLY the most important element
        for element in content_elements[:1]:  # Take only the most important content element per step
        ...
        # Add ONLY the most critical conversion-focused behaviors
        for indicator in conversion_indicators[:1]:  # Take only the top conversion indicator per step
            if indicator and "click" not in indicator.lower() and len(indicator.strip()) > 3:
                all_behaviors.append({
                    "behavior": indicator.strip(),
                    "step_context": step_name,
                    "friction_score": 4,
                    "priority": 8
                })
    
    # Add specific CTA-based behaviors if we have room (targeting under 10 total)
    cta_behaviors_added = 0
    for cta in top_ctas:
        if len(all_behaviors) < 8 and cta_behaviors_added < 2:  # Leave room and don't overdo CTAs
    ...
    # Add essential behaviors if we don't have enough (aim for 6-9 total)
    essential_behaviors = [
    ...
    for essential in essential_behaviors:
        if len(all_behaviors) < 8:  # Only add if we need more to reach target
    ...
    # Sort by priority (highest first) and limit to 9 behaviors maximum (strictly below 10)
    all_behaviors.sort(key=lambda x: x['priority'], reverse=True)
    key_behaviors = all_behaviors[:9]
